...
def load_model(model_fname = './src/model/xgboostModel'):
...
def predict_on_one_batch_and_write(sORFs, logr, threshold, output_fname, k=4):
  ...
  # Assign classification based on y_pred value
  classification = []
  for val in y_pred:
    if np.isnan(val):
      classification.append('Not classified')
    elif val == 0:
      classification.append('noncoding')
    elif val == 1:
      classification.append('coding')
    else:
      classification.append('Not classified')
